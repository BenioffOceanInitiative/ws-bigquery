...
-- 		SELECT
-- 			(SAFE_CAST ('1990-01-01' AS TIMESTAMP)));
...
DECLARE
	new_vsr_seg_agg_ts DEFAULT(
		SELECT
			CASE WHEN (
				...
				SAFE_CAST('2020-01-01 00:00:00 UTC' AS TIMESTAMP)
			ELSE
				(
					SELECT
						MAX(timestamp_end)
	...
	WHERE
		...
		AND s.date > DATE(new_vsr_seg_agg_ts)
		AND s.date <= (
			SELECT
	...
	PARTITION BY date CLUSTER BY
		...
		SELECT
			(new_vsr_seg_agg_ts) AS newest_timestamp,
